...
def index():
    ...
    symbols = db.execute("SELECT shares, symbol \
                                    FROM portfolio WHERE id = :id", \
                                    id=session["user_id"])
    
    total_cash = 0
   
    for portfolio_symbol in symbols:
    ...
    upd_cash = db.execute("SELECT cash FROM users \
                               WHERE id=:id", id=session["user_id"])
    
   
    total_cash += upd_cash[0]["cash"]
    ...
    return render_template("index.html", stocks=updated_portfolio, \
                            cash=usd(upd_cash[0]["cash"]), total= usd(total_cash) )
